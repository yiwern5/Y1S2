...
/**
 * An improved version of the MazePrinter, where the rooms and walls are more correctly and
 * neatly represented. Also, this ImprovedMazePrinter also searches for the source of the
 * path in the Maze, then traces this path to determine the route the MazeSolver took, and
 * finally print out the Maze with the proper walls correctly destroyed. (It is hard to decide
...
public class ImprovedMazePrinter {
	// static class
	private ImprovedMazePrinter() {
	}

	/**
	 * Storing the different directions each move can take, and their corresponding offset.
	 */
	...
	/**
	 * Binary logic for determining what type of wall to draw
	 * 4-bits: NSEW. Each bit is a boolean indicating if there is a wall in that direction
	 * at the current "corner".
	 */
	private static final String[] CORNERS = new String[]{
			PrinterBlocks.AIR.val(),             // 0000
	...
	/**
	 * Binary logic for determining what type of path to draw
	 * Four bits: MSB 0 = northRoom.onPath
	 * 1 = southRoom.onPath
	 * 2 = eastRoom.onPath
	 * LSB 3 = westRoom.onPath
	 */
	private static final String[] PATHS = new String[]{
			PrinterBlocks.PATH_VERTICAL.val(),     // 0000
			null,                                 // 0001
	...
	/**
	 * Binary logic to determine what horizontal gap to draw
	 * 2-bit: MSB 0 = room on path
	 * LSB 2 = wall in that direction
	 */
	private static final String[] HORZ_GAPS = new String[]{
			PrinterBlocks.AIR.val(),
			PrinterBlocks.WALL_VERTICAL.val(),
			PrinterBlocks.PATH_HORIZONTAL.val(),
			PrinterBlocks.DESTROY_VERTICAL.val()
	};

	private static final String[] VERT_GAPS = new String[]{
			PrinterBlocks.AIR.val(),
			PrinterBlocks.WALL_HORIZONTAL.val(),
			PrinterBlocks.PATH_VERTICAL.val(),
			PrinterBlocks.DESTROY_HORIZONTAL.val()
	};

	/**
	 * Determines starting point if needed, then build the "walls" of the maze first.
	 * Then, back-trace the path found, and build the appropriate directions
	 * Finally, print out the maze and the path
	 *
	 * @param maze
	 * @param startRow
	 * @param startCol
	...
	static void printMaze(Maze maze, Integer startRow, Integer startCol) {
		...
		// pathRatio = (numOfOutgoingPaths - numOfWallsInThosePaths) * numOfOutgoinPaths;
		// claim: the starting point (or ending point, interchangeable) has the minimum pathRatio!
		// therefore, iterate through all rooms and calculate their pathRatio, and keep track of 
		// the room with minimum pathRatio.
		Integer pathRatio = Integer.MAX_VALUE;
		// pathGraph has Coordinates (or Rooms) as vertices, and each outgoing edge contains a valid
		// (but may not be the correct path) pointer to adjacent rooms. Using AdjacencyList method.
		HashMap<Coordinates, LinkedList<DirectedEdge>> pathGraph = findSource ?
				new HashMap<Coordinates, LinkedList<DirectedEdge>>() : null;

		// mazeCorners contains the appropriate PrinterBlocks to draw for the the walls of the maze
		// claim: if a maze has n rows and m columns, then there are n+1 horizontal walls and m+1
		// vertical walls
		String[][] mazeCorners = new String[maze.getRows() + 1][maze.getColumns() + 1];

		// first iteration: does essentially two things at once
		// 1. if findSource == true, then see if room(i, j) is onPath and enumerate all possible
		// 	  outgoin paths, store it in pathGraph
		...
		for (int i = 0; i < maze.getRows() + 1; i++)
			for (int j = 0; j < maze.getColumns() + 1; j++) {
				...
				if (findSource && i < maze.getRows() && j < maze.getColumns() && (r = maze.getRoom(i, j)).onPath) {
					...
					// keep track of outgoing edges, in terms of Directed Edges (called DirectedEdges)
					...
					for (Direction d : Direction.values()) {
						...
						if (c.getX() < 0 || c.getX() >= maze.getRows() ||
								c.getY() < 0 || c.getY() >= maze.getRows()) {
							continue;
						}
						// don't bother with rooms that are not onPath
						if (!maze.getRoom(c.getX(), c.getY()).onPath) continue;
						...
						boolean hasWall = d == Direction.NORTH && r.hasNorthWall() ||
								d == Direction.SOUTH && r.hasSouthWall() ||
								d == Direction.EAST && r.hasEastWall() ||
								d == Direction.WEST && r.hasWestWall();
						if (hasWall) wallsInPath++;
				...
				if (i == 0 && j == 0)
					...
					boolean south = i < maze.getRows() && (j <= 0 || j >= maze.getColumns() || maze.getRoom(i, j).hasWestWall());
					boolean east = j < maze.getColumns() && (i <= 0 || i >= maze.getRows() || maze.getRoom(i - 1, j).hasSouthWall());
					boolean west = j > 0 && (i <= 0 || i >= maze.getRows() || maze.getRoom(i, j - 1).hasNorthWall());
					// 4-bits: NSEW
					int corner = (north ? 1 : 0) * 8 +
							(south ? 1 : 0) * 4 +
							(east ? 1 : 0) * 2 +
							(west ? 1 : 0);
		...
		while (currentPosition != null) {
			...
			// Determine next room
			if (findSource) {
				// If source is automatically determined, then just iterate through the LinkedList of valid
				...
				for (Iterator<DirectedEdge> dir_it = pathGraph.get(currentPosition).iterator(); dir_it.hasNext(); ) {
					...
					if (roomPath.containsKey(c)) continue;
					if (v.getValue() < w) {
						d = dir;
						w = v.getValue();
					}
				...
				// If source is given, then the source search has not been conducted and there are no LinkedLists
				// to refer to to find the valid rooms. We have to do this ourselves now instead.
				for (Direction dir : Direction.values()) {
					// Get coords of the room in this direction
					Coordinates c = new Coordinates(currentPosition, dir);
					// Check if room is out of bounds
					if (c.getX() < 0 || c.getX() >= maze.getRows() ||
							c.getY() < 0 || c.getY() >= maze.getColumns()) continue;
		...
		if (path_it.hasNext()) {
			...
			while (path_it.hasNext()) {
				...
				// if current ≠ start, use Binary logic magic
				current.setValue(PATHS[current.getArrivedVia().ordinal() * 4 + nextBlock.getArrivedVia().ordinal()]);
				current = nextBlock;
			}
			// Modify end point's block according to the incoming direction, purely for cosmetic reasons
		...
		for (int i = 0; i < mazeCorners.length; i++) {
				...
				if (j < mazeCorners[i].length - 1) {
					...
					int gapCode = (roomBlock != null &&
							(roomPath.containsKey(new Coordinates(i - 1, j)) && roomBlock.arrivedVia(Direction.SOUTH) ||
									upperBlock != null && upperBlock.arrivedVia(Direction.NORTH))
							? 1 : 0) * 2 +
							(i == 0 || i > 0 && maze.getRoom(i - 1, j).hasSouthWall() ? 1 : 0);
			...
			if (i < mazeCorners.length - 1) {
				...
				for (int j = 0; j < maze.getColumns(); j++) {
					...
					Block roomBlock = roomPath.get(new Coordinates(i, j));
					if (roomBlock != null) roomString = roomBlock.getValue();
	...
	/**
	 * Storing the PrinterBlock string for the appropriate incoming direction m_arrivedVia
	 */
	...
	/**
	 * Anti-Cartesian coordinates, as x is the vertical axis while y is the horizontal axis
	 * Downwards (south) and leftwards (east) are considered positive
	 */
	private static class Coordinates {
		...
		// Can obtain a new coodinates using a base coordinates and an offset
		...
		@Override
		public int hashCode() {
			if (this.m_hash != null) return this.m_hash;
			int h = 7;
			h = h * 31 + this.m_x;
			h = h * 257 + this.m_y;
			this.m_hash = h;
		...
		public boolean equals(Object o) {
			...
			if (this.m_x.intValue() != c.m_x.intValue()) return false;
			if (this.m_y.intValue() != c.m_y.intValue()) return false;
			return true;
		}
	}
}
